...
export async function fetchOffersOverviewRows(): Promise<{
  ...
  if (!admin) return { offers: [], migrationApplied: true };

  const { data, error } = await admin
    .from("customer_offers")
    .select(
      "id, customer_id, title, product_id, discount_kind, discount_value, is_active, starts_at, ends_at, customers(business_name), products(name, sku)",
    )
    .order("created_at", { ascending: false })
    .limit(1000);
  if (error) {
    if (error.code !== "42P01") console.error("[admin] offers overview read failed:", error.message);
    return { offers: [], migrationApplied: error.code !== "42P01" };
  }
